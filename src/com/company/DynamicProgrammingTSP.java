...
class DynamicProgrammingTSP {
    ...
    private double[][] graph; // Representing the adjacency matrix of the graph
    ...
    //List<Integer> intList = convert_v();
    ...
    DynamicProgrammingTSP(double[][] graph) {
        this.n = graph.length;
        ...
        //this.shortestPath = new ArrayList<Integer>(){{ for (int i : v) add(i); }};
        //this.shortestPath = convert_v();
        this.shortestPath = new ArrayList<>();
    }


    List<Integer> convert_v(){
        List<Integer> intList = new ArrayList<Integer>(v.length);
        for ( int i : v){
            intList.add(i);
        }
        return intList;
    ...
    double getShortestPathCost()
    {
            ...
            for (int subset : combinations(r, n))
            {
                ...
                for (int next = 0; next < n; next++) {
                    ...
                    for (int end = 0; end < n; end++) {
                        ...
                        double newDistance = memory[end][subsetWithoutNext] + graph[end][next];
        ...
        for (int i = 0; i < n; i++) {
            if (i == root) continue;
            double tourCost = memory[i][endState] + graph[i][root];
